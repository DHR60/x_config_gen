...
abstract class FinalMask4Ray with _$FinalMask4Ray {
  ...
  const factory FinalMask4Ray({List<Mask4Ray>? tcp, List<Mask4Ray>? udp}) =
      _FinalMask4Ray;
...
abstract class Mask4Ray with _$Mask4Ray {
  @JsonSerializable(includeIfNull: false, explicitToJson: true)
  const factory Mask4Ray({required String type, MaskSettings4Ray? settings}) =
      _Mask4Ray;
...
abstract class MaskSettings4Ray with _$MaskSettings4Ray {
  ...
  const factory MaskSettings4Ray({
    String? password,
    String? domain,
    String? listenIp,
    int? id,
